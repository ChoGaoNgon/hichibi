...
### Quản lý Cache:
1.  Hệ thống sử dụng `localStorage` để cache một số dữ liệu tĩnh (Danh mục, Sản phẩm) và Access Token của Google Calendar nhằm tăng tốc độ tải trang và duy trì phiên làm việc.
